...
namespace DongZheng.H3.WebApi.Common.Util
{
    public class QRCodeGenerator
    {
        ...
        public static Bitmap GeneratorQrImage(string contents, Image middleImg, Color foreColor, int margin, int width = 300, int height = 300)
        {
            ...
            barcodeWriter.Renderer = new ZXing.Rendering.BitmapRenderer { Foreground = foreColor };
        ...
        /// <returns></returns>
        public static string DecodeQrImage(Bitmap barcodeBitmap)
        {
            //构建解码器
        ...
        {
            path = path ?? "Icon.png";//.MapRootPath(true);

            Image LogImg = Image.FromFile(path);

            return LogImg;
        }

        public static System.IO.MemoryStream GeneratorShopQrCode(string contents, string logoPath, Color foreColor, int margin, int width = 300, int height = 300)
        {
            var logo = GetLogo(logoPath);

            var img = GeneratorQrImage(contents, logo, foreColor, margin, width, height);
        ...
        public static System.IO.MemoryStream GeneratorShopQrCode(string contents)
        {
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            Bitmap bitmap = GeneratorQrImage(contents);
