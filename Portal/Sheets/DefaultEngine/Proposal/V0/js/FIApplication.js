...
function addressFieldShow(id, type) {
    ...
    address.find("div.rows").each(function (n, v) {
        ...
        if ($.MvcSheetUI.GetControlValue("ADDRESS.IDENTIFICATION_CODE4", index) == id) {
            if (type == "00001") {
                //显示家庭地址、籍贯、出生省市县
                $.MvcSheetUI.GetElement("ADDRESS.ADDRESS_ID", index).parent().parent().show();
                $.MvcSheetUI.GetElement("ADDRESS.NATIVE_DISTRICT", index).parent().parent().parent().show();
                //隐藏注册地址
                $.MvcSheetUI.GetElement("ADDRESS.REGISTERED_ADDRESS", index).parent().parent().hide();
                ...
                //隐藏家庭地址、籍贯、出生省市县
                $.MvcSheetUI.GetElement("ADDRESS.ADDRESS_ID", index).parent().parent().hide();
                $.MvcSheetUI.GetElement("ADDRESS.NATIVE_DISTRICT", index).parent().parent().parent().hide();
                //显示注册地址
                $.MvcSheetUI.GetElement("ADDRESS.REGISTERED_ADDRESS", index).parent().parent().show();
...
function validate_marital(applicant_detail_rows) {
    ...
    if (t == "00001") {//个人
        applicant_detail_rows.each(function (n, v) {
            var v = $.MvcSheetUI.GetControlValue("APPLICANT_DETAIL.IDENTIFICATION_CODE2", $(v).attr("data-row"));
            if (v == "1") {
                app_marital_code = $.MvcSheetUI.GetControlValue("APPLICANT_DETAIL.MARITAL_STATUS_CDE", $(v).attr("data-row"));
                app_det_index = n;
                return false;
            }
        });
    ...
    applicant_detail_rows.each(function (n, v) {
        ...
        if (n == app_det_index) {
            ...
            if (info.MAIN_APPLICANT == "Y") {
                mainMaritalCode = $.MvcSheetUI.GetControlValue("APPLICANT_DETAIL.MARITAL_STATUS_CDE", index);
            }
        ...
        if (info.APPLICANT_TYPE == "I") {//表示是共同借款人
            ...
            //2.再检查借款人与共借人关系与婚姻状态
            if (relation == "00006" && marital_code == "00001" && mainMaritalCode != "00001") {
                msg += Message.Applicant_Info_17;
                return false;
            }
            ...
            if (relation == "00006" && marital_code == "00001" && mainMaritalCode == "00001" && parseFloat(financedAmt) > 300000) {
                //检查共借人是否隐藏
...
function validate_IdentifyCode(applicant_detail_rows, company_detail_rows) {
    ...
    applicant_detail_rows.each(function (n, v) {
        ...
        $(v).find("input[data-datafield='APPLICANT_DETAIL.FIRST_THI_NME']").change();//触发change();
    ...
    company_detail_rows.each(function (n, v) {
        ...
        $(v).find("input[data-datafield='COMPANY_DETAIL.COMPANY_THI_NME']").change();//触发change();
        $(v).find("input[data-datafield='COMPANY_DETAIL.ORGANIZATION_CDE']").change();//触发change();
        if (!IdentifyCodeChange(con)) {
            result = false;
            return false;
        }
...
function setAddressHiddenField(tab_a) {
    ...
    if (info.APPLICANT_TYPE == "I" || info.GUARANTOR_TYPE == "I") {//个人
        $.MvcSheetUI.GetElement("ADDRESS.REGISTERED_ADDRESS", index).parent().parent().hide();//隐藏注册地址

        $.MvcSheetUI.GetElement("ADDRESS.ADDRESS_ID", index).parent().parent().show();//显示家庭地址
        $.MvcSheetUI.GetElement("ADDRESS.NATIVE_DISTRICT", index).parent().parent().parent().show();//显示籍贯, 出生地省市县
        ...
        $.MvcSheetUI.GetElement("ADDRESS.ADDRESS_ID", index).parent().parent().hide();//隐藏家庭地址
        $.MvcSheetUI.GetElement("ADDRESS.NATIVE_DISTRICT", index).parent().parent().parent().hide();//隐藏籍贯, 出生地省市县

        $.MvcSheetUI.GetElement("ADDRESS.REGISTERED_ADDRESS", index).parent().parent().show();//显示注册地址
