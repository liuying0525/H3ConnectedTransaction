...
{
    public partial class FIApplication_Mobile : OThinker.H3.Controllers.MvcPage
    {
        ...
        public object get_HK_plan(string total_price, string dkje, string wkje, string dkqs, string product_id)
        {
            ...
            double para_t = 0;
            double para_r = 0;//客户利率
            double para_actual_rte = 0;//实际利率
            ...
            {
                para_r = Convert.ToDouble(dt_r.Rows[0]["customer_rte"]);
                para_actual_rte = Convert.ToDouble(dt_r.Rows[0]["actual_rte"]);
            }
            ...
            foreach (DataRow row in dt_plan.Rows)
            {
                ...
                else
                {
                    ...
                    plan_simple.Add(new
                    {
                        ...
                        INTEREST_RTE = (para_r * 100).ToString("0.00")
                    });
                    //重启记录一条新的数据
            ...
            return new
            {
                Success = true,
                Plan_ID = plan_id,
                RTE = (para_r * 100).ToString("0.00"),
                ACTUAL_RTE = para_actual_rte.ToString("0.00"),
                TX_RTE = (para_actual_rte - para_r * 100).ToString("0.00"),
                PlanInfo = plan_info,
