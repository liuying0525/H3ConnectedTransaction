module.controller('startworkflowCtrl', function ($scope, $rootScope, categories, $ionicSlideBoxDelegate, $ionicLoading, commonJS) {
    ...
    $scope.loadWorkflows = function () {
        ...
        categories.all(params).then(function (result) {
            ...
            if (result.Workflows) {
                ...
                if ($scope.FavoriteNum == 0 || $scope.slectIndex==1) {
                    $scope.activeSlide(1);
                } else {
                    $scope.activeSlide(0);
                }
    ...
    $scope.setFavorite = function (workflow) {
        ...
        var params = null;
        if (window.cordova) {
            url = "/SetFavorite?callback=JSON_CALLBACK";
        ...
        console.log(params);
        categories.setFavorite(url, params).then(function (result) {
            console.log(params);
        });
    }
    $scope.init();
