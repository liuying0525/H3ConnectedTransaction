services.service("commonJS", function ($rootScope, $ionicModal, $http, $ionicLoading, $cordovaToast, $state,
    ...
    this.GetWorkItemSheetUrl = function ($scope, url, workItemId) {
        ...
        .success(function (data) {
            if (data.Success) {
                var url = "";
                var index=data.Message.toLocaleLowerCase().indexOf(config.portalroot.toLocaleLowerCase());
                if (index >= 0 && data.Message.toLocaleLowerCase().substr(index, config.portalroot.toLocaleLowerCase().length) == config.portalroot.toLocaleLowerCase()) {
                    url = $scope.setting.httpUrl.toLocaleLowerCase().split(config.portalroot.toLocaleLowerCase())[0] + data.Message;
                }
                else {
                    url = $scope.setting.httpUrl.toLocaleLowerCase() + "/" + data.Message;
                }
    ...
    // sideSlip侧滑框
    this.sideSlip = function ($scope, templates) {
        $rootScope.searchItem = true;
        // .fromTemplateUrl() 方法
        $ionicPopover.fromTemplateUrl(templates, {
            scope: $scope
