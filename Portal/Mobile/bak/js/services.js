services.service("commonJS", function ($rootScope, $ionicModal, $http, $ionicLoading, $cordovaToast, $state,
                                 $ionicPopup, $cordovaFileTransfer, $cordovaFileOpener2, $timeout, $cordovaNetwork, $ionicPopover) {
      ...
      this.sideSlip = function($scope,templates){
        ...
        //选中
        $scope.clickMergency = function ($event) {
            if($event.target.nodeName=="INPUT"){
                 for (var i = 0; i < $event.target.parentElement.children.length; i++) {
                   $event.target.parentElement.children[i].className = "f14";
                            }
             $event.target.className = 'f14 on';
            }
        };
        //清空
