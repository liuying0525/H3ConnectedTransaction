    ...
    $.MvcSheet,
    {
        ...
        LoadData: function (_AfterMvcInit) {
            ...
            this.GetSheet(
                ...
                function (data) {
                    ...
                    //增加Copy功能
                    var copyid = $.MvcSheetUI.QueryString("CopyID")
                    if (copyid != null && copyid != undefined && copyid != "" && data.IsOriginateMode) {
                        $.ajax({
                            //url: "/Portal/Custom/ajax/CommonHandler.ashx",
                            url: "/Portal/CommonHandler/Index",//wangxg 19.7
                            data: { CommandName: "loadmvcdata", id: copyid, schemacode: data.SchemaCode, T: new Date().getTime() },
                            type: "POST",
                            async: false,
                            dataType: "json",
                            success: function (res) {
                                ...
                                if (res.Success) {
                                    var result = res.Data;
                                    for (var k in result) {
                                        if (result[k].L == "26") {//参与者单人
                                            if (result[k].V == null || result[k].V == undefined) {
                                                data.BizObject.DataItems[k].V = null;
                                            }
                                            else {
                                                data.BizObject.DataItems[k].V = {};
                                                data.BizObject.DataItems[k].V["Code"] = result[k].V.Code;
                                                data.BizObject.DataItems[k].V["Name"] = result[k].V.Name;
                                                data.BizObject.DataItems[k].V["ContentType"] = result[k].V.ContentType;
                                                data.BizObject.DataItems[k].V["Size"] = 0;
                                                data.BizObject.DataItems[k].V["Url"] = null;
                                            }
                                        }
                                        if (result[k].L == "27") {//参与者多人
                                            if (result[k].V == null || result[k].V == undefined) {
                                                data.BizObject.DataItems[k].V = null;
                                            }
                                            else {
                                                data.BizObject.DataItems[k].V = [];
                                                result[k].V.forEach(function (cyz_v, cyz_n) {
                                                    var obj = {};
                                                    obj["Code"] = cyz_v.Code;
                                                    obj["Name"] = cyz_v.Name;
                                                    obj["ContentType"] = cyz_v.ContentType;
                                                    obj["Size"] = 0;
                                                    obj["Url"] = null;
                                                    data.BizObject.DataItems[k].V.push(obj);
                                                });
                                            }
                                        }
                                        else if (result[k].L == "41" && data.BizObject.DataItems[k].L == "41") {//子表
                                            data.BizObject.DataItems[k].V.R.splice(0, data.BizObject.DataItems[k].V.R.length);

                                            result[k].V.R.forEach(function (v, n) {
                                                var t = $.MvcSheet.Clone(data.BizObject.DataItems[k].V.T);
                                                var obj_id = $.MvcSheetUI.NewGuid();
                                                for (var c in t.DataItems) {
                                                    if (c.indexOf("ObjectID") > 0) {
                                                        t.DataItems[c].V = obj_id;
                                                    }
                                                    else {
                                                        t.DataItems[c].V = v.DataItems[c].V;
                                                        t.DataItems[c]["BizObjectID"] = obj_id;
                                                    }
                                                }
                                                data.BizObject.DataItems[k].V.R.push(t)
                                            });
                                        }
                                        else if (result[k].L == "13" ||//长文本
                                            result[k].L == "11"||//长整数
                                            result[k].L == "14"||//短文本
                                            result[k].L == "30"||//HTML
                                            result[k].L == "1"||//逻辑型
                                            ...
                                            result[k].L == "7" ||//数值类型
                                            result[k].L == "9") {//整数类型
                                            data.BizObject.DataItems[k].V = result[k].V;//直接给V进行赋值
                                        }
                                        else {//其它类型不做处理:附件，审批意见，链接，全局变量，关联表单
                                            continue;
                                        }
                                    }
                                }
                            ...
                            error: function (msg) {
                                console.log("Copy InstanceData Error:" + msg);
                            }
                        });
                    }
                    ...
                    if ($.MvcSheetUI.SheetInfo.PageScript)h
                    {
                    ...
                    if ($.MvcSheetUI.SheetInfo.IsMobile) {
                        ...
                        $("div[class='row']").each(function () {
                            //if ($(this).find("div:visible[class^='col-md-']").length == 0) {//chenghs 2018-04-09
                            if ($(this).find("div[class^='col-md-']").length == 0) {
                                $(this).hide();
                            }
                        });
                    }
        ...
        Clone: function (obj) {
            ...
            if (obj instanceof Object) {
                var copy = {};
                for (var attr in obj) {
                    if (obj.hasOwnProperty(attr)) copy[attr] = $.MvcSheet.Clone(obj[attr]);
                }
                return copy;
            }
        ...
        Action: function (actionControl) {
            ...
            var callResult = true;
            if ($.isFunction(actionControl.OnAction)) {//javascript函数
                callResult = actionControl.OnAction.apply(this);
            }
            else if (actionControl.OnAction) {//javascript语句
                callResult = new Function(actionControl.OnAction).apply(this);
            }
            if (!callResult) {
                $.LoadingMask.Hide();
                return false;
        ...
        FinishInstance: function (actionControl) {
            ...
            //增加一个结束流程并确认后的事件，此方法在许多需要与其它系统做集成的时候非常有用；Add by chenghs 2018-07-12
            var callResult = true;
            if ($.isFunction(that.AfterConfirmFinish)) {//javascript函数
                callResult = that.AfterConfirmFinish.apply(that);
            }
            else if (that.AfterConfirmFinish) {//javascript语句
                callResult = new Function(that.AfterConfirmFinish).apply(that);
            }
            if (callResult) {
                that.PostSheet(
