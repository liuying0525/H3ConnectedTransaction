    ...
    $.MvcSheetUI.Controls.SheetDropDownList.Inherit($.MvcSheetUI.IControl, {
            ...
            if (this.SchemaCode.trim() != "") { //业务对象数据源（主数据）
                ...
                if ($.MvcSheetUI.CacheData && $.MvcSheetUI.CacheData[cmdParam]) {
                    ...
                    for (var key in cacheData) {
                        $element.append("<option value=\"" + cacheData[key].key + "\">" + cacheData[key].value + "</option>");
                        if (this.IsMobile) {
                            this.AddMobileItem(cacheData[key].key, cacheData[key].value, false);
                        }
                    }
                        ...
                        $.MvcSheet.GetSheet(
                            ...
                            function (data) {
                                ...
                                if (data.Successful != null && !data.Successful) {
                                    ...
                                    for (var key in data) {
                                        if (!$element.find("option[value='" + data[key].key + "']") || $element.find("option[value='" + data[key].key + "']").length == 0) {
                                            $element.append("<option value=\"" + data[key].key + "\">" + data[key].value + "</option>");
                                            if (that.IsMobile) {
                                                that.AddMobileItem(data[key].key, data[key].value, false);
                                            }
                                        }
                                    }
                ...
                if ($.MvcSheetUI.CacheData && $.MvcSheetUI.CacheData[cmdParam]) {
                    var cacheData = $.MvcSheetUI.CacheData[cmdParam];
                    for (var key in cacheData) {
                        if (cacheData[key].Sort == 10000 && $.MvcSheetUI.SheetInfo.StartActivityCode == $.MvcSheetUI.SheetInfo.ActivityCode && ($.MvcSheetUI.SheetInfo.IsOriginateMode || $.MvcSheetUI.SheetInfo.SheetMode == $.MvcSheetUI.SheetMode.Work)) { }
                        else {
                        ...
                        function (data) {
                            if (data) { 
                                ...
                                for (var i = 0, len = data.length; i < len; i++) {
                                    if (data[i].Sort == 10000 && $.MvcSheetUI.SheetInfo.StartActivityCode == $.MvcSheetUI.SheetInfo.ActivityCode && ($.MvcSheetUI.SheetInfo.IsOriginateMode || $.MvcSheetUI.SheetInfo.SheetMode == $.MvcSheetUI.SheetMode.Work) ) { }
                                    else {
        ...
        ionicInit: function (ionic) {
            ...
            $(this.Element.parentElement).unbind('click.showChoice').bind('click.showChoice', function (e) {
                    ionic.$ionicModal.fromTemplateUrl('/Portal/Mobile/form/templates/radio_popover.html', {
                    scope: ionic.$scope
