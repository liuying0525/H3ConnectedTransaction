...
(function ($) {
    ...
    $.MvcSheetUI.Controls.SheetAttachment.Inherit($.MvcSheetUI.IControl, {
        Render: function () {
            ...
            //判断是否启用OCR识别
            if (eval(this.OCR)) {
                this.SheetAttachmentHandler = this.OCRUrl;
            }
            else if (eval(this.PictureCompress)) {//判断是否启用图片压缩
                this.SheetAttachmentHandler = this.PictureCompressUrl;
            }
            //是否支持Html5
            if ((this.IsHtml5)) {
                this.Html5Render();
        ...
        MobileItemClick: function (fileId, fileUrl, IsImg) {
            var _that = this;
            var buttons = [
                { text: SheetLanguages.Current.Download, code: 'download' },
            ...
            if (fileUrl && !this.Editable) {
                buttons = [{ text: SheetLanguages.Current.Download, code: 'download' }];
            };
            var hideSheet = $.MvcSheetUI.IonicFramework.$ionicActionSheet.show({
                buttons: buttons,
        ...
        CreateFileElement: function (fileid, name, size, url, contentType, actionName, isCompress) {
            actionName = actionName || SheetLanguages.Current.Loading;
            var fileSizeStr = 0;
            if (size > 1024 * 1024)
                fileSizeStr = (Math.round(size * 100 / (1024 * 1024)) / 100).toString() + 'MB';
            else
                fileSizeStr = (Math.round(size * 100 / 1024) / 100).toString() + 'KB';
            var fileSize = $("<td data-filesize='" + fileid + "'><span data-filerate='" + fileid + "'>" + actionName + "</span> (" + fileSizeStr + ")</td>").addClass("text-info");
            var actionTd = $("<td data-action='" + fileid + "' class=\"printHidden\"></td>");

            //移动端使用span
            if (this.IsMobile) {
                fileSize = $("<span class='item-note' data-filesize='" + fileid + "'>（" + fileSizeStr + "）</span><span class='item-note' data-filerate='" + fileid + "'>" + actionName + "</span>")
            ...
            if (this.Editable) {
                actionStr.unbind("click.fileDeleteBtn").bind("click.fileDeleteBtn", this, function (e) {
                    var flag = isCompress;
                    if (flag || confirm(SheetLanguages.Current.ConfirmRemove)) {
        ...
        AddFiles: function (files) {
            ...
            if (files && files.length > 0) {
                ...
                var picCompressQuality = this.PictureCompressQuality;
                var picCompressByJs = eval(this.PictureCompressByJs);
                var uploadFile = function (fileid, file) {
                    if (self.CreateFileElement(fileid, file.name, file.size, null, file.type)) {
                        self.AddAttachments[fileid] = {
                            fileid: fileid,
                            file: file,
                        ...
                        files.splice(0, 1);
                        self.UploadFile(fileid, files);
                    }
                };
                var removeFileElement = function (id) {
                    if (self.IsMobile) {
                        $(id).remove();
                    } else {
                        $(id).find('a.fa-minus').click();
                    }
                };
                ...
                // addby zhangfl 2019/09/19 图片上传之前先压缩
                if (picCompressByJs && isImage(files[0].name) && ImageCompressor && files[0].size > 1024 * 1024 * 1) {
                    // 在界面上显示正在压缩...，提升体验
                    var compressFileId = $.MvcSheetUI.NewGuid();
                    self.CreateFileElement(compressFileId, files[0].name, files[0].size, null, files[0].type, SheetLanguages.Current.Compressing, true);

                    // 设置图片压缩质量
                    if (files[0].size < 1024 * 1024 * 2) {
                        picCompressQuality = 0.8;
                    } else if (files[0].size < 1024 * 1024 * 3) {
                        picCompressQuality = 0.6;
                    } else {
                        picCompressQuality = 0.4;
                    }

                    new ImageCompressor(files[0], {
                        quality: picCompressQuality,
                        success: function (blob) {
                            removeFileElement('#' + compressFileId);

                            // 上传到服务器需要使用File对象，这里需要将Blob对象转为File对象
        ...
        UploadFile: function (fileid, files) {
            ...
            var fd = new FormData();
            if (eval(this.OCR)) {//启用OCR识别
                fd = eval(this.OCRUrlParameter);
            }
            else if (eval(this.PictureCompress)) {//启用图片压缩
                fd = eval(this.PictureCompressUrlParameter);
            }
        ...
        UploadComplete: function (evt) {
            if (evt.target.status == 200) {
                ...
                if (resultObj.OCR_Result) {//如果启用OCR
                    if (resultObj.OCR_Result.code != "0") {
                        this.context.AddAttachments[fileid].state = 100;
                        $("span[data-filerate='" + fileid + "']").html(resultObj.OCR_Result.message).css("color", "red");
                        return false;
                    }
                }
