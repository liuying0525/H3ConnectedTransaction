...
SheetDesigner.DropContainer.prototype = {
    ...
    BindControl: function (container, datafield, displayName, controlId, controlType) {
        if (controlType == "SheetGridView") {// 处理子表
        ...
        else if (controlType == "SheetDetail") {// 处理子表SheetDetail add by chenghs 2018-04-09
            var li = this.Controls.treeData.find("span[data-datafield='" + datafield + "']").parent().parent().parent();
            var childrens = li.find("span[data-datafield]").filter(function () { return $(this).attr("data-datafield") != datafield; });

            var tableHtml = "<div id=\"" + controlId + "\" data-datafield=\"" + datafield + "\" data-type=\"SheetDetail\" class=\"SheetGridView\">";
            tableHtml += "<div class=\"OnlyDesigner\"><div colspan=\"" + (childrens.length + 2) + "\" class=\"OnlyDesigner\">";
            tableHtml += "<label data-datafield=\"" + datafield + "\" data-for=\"" + controlId + "\" data-type=\"SheetDetail\">" + datafield + "属性</label>";
            tableHtml += "</div></div>";

            tableHtml += "<ul id=\"myTab_" + controlId + "\" class=\"nav nav-tabs\"></ul>";

            tableHtml += "<div id=\"myTabContent_" + controlId + "\" class=\"tab-content\">";
            tableHtml += "<div class=\"SheetGridViewTemplate\">";
            ...
            for (var i = 0; i < childrens.length; i++) {
                ...
                var displayName = ctl.attr("data-displayname");
                var logicType = ctl.attr("data-logictype");
                if (logicType == "24") {//如果是附件，独占一行
                    if (rowcount != 0) {
                        tableHtml += "</div></div></div>";//结束这一行
                        tableHtml += "<div class=\"row\">";//重新开一行；
                    }
                    isOneRow = true;
                    rowcount = 2;//设置为2，会重新再创建新行
                }
                else {
                    isOneRow = false;
                }

                tableHtml += "<div class=\"col-md-" + (isOneRow ? "12" : "4") + "\">";

                tableHtml += "<div id=\"" + controlId + "_Title" + i + "\" class=\"col-md-4\">";
                tableHtml += "<span id=\"" + controlId + "_label" + i + "\">" + displayName + "</span>";
                tableHtml += "</div>";
                tableHtml += "<div id=\"" + controlId + "_Data" + i + "\" class=\"col-md-8\">";
