...
function jumpToEidtDoc(Type, isNew) {
    ...
    if (schemacode == "Solutionmortgage") {
        ContractNumber = /*"Br-A083522000";*/$.MvcSheetUI.SheetInfo.BizObject.DataItems["ApplyNo"].V;
    }
    if (schemacode == "AllowIn") {
        ContractNumber = /*2883*/$.MvcSheetUI.SheetInfo.BizObject.DataItems["License"].V;
    }
    ...
    if ($("#select2 option:selected").text() == "营业执照") {
        $.ajax({
            type: "Post",
            url: "/Portal/Ajax/DistributorLicense.ashx",
            data: { Distributor: $.MvcSheetUI.SheetInfo.BizObject.DataItems["ApplyNo"].V, action: "Get" },
            success: function (g) {
                if (g == "") alert("没有找到营业执照");
                else {
                    var t = window.open("http://localhost:8010/Portal/Filetemplate/营业执照/" + g);
                    t.window.print();
                }
            }
        });
...
function GetfileGuid(key) {
    ...
    var fileGuid = "";
    //检查之前文件是否生成过
    if (CheckKeyJson != "" && CheckKeyJson != null) {
        JSONstr = eval('(' + CheckKeyJson + ')');
...
function SheetLoaded() {
    ...
    if ((SchemaCode == "Solutionmortgage" || SchemaCode == "AllowIn") && !IsMobile) {
        $.MvcSheet.Action({
