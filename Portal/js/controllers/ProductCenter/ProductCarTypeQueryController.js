app.controller('ProductCarTypeQueryController', ['$scope', "$rootScope", "$translate", "$timeout", "$compile",
    ...
    function ($scope, $rootScope, $translate, $timeout, $compile, $http, $state, $stateParams, $filter, ControllerConfig, $interval, jqdatables) {
        ...
        $scope.options = function () {
            var options = {
                ...
                "sAjaxSource": "ProductCenter/ProductCarTypeQuery",
                "fnServerData": function (sSource, aDataSet, fnCallback) {

                    if (($scope.SearchKey.ProductName == "" || $scope.SearchKey.ProductName == null) && ($scope.SearchKey.CarType == "" || $scope.SearchKey.CarType == null)) {
                        $.notify({ message: "请输入产品名称或车辆款型", status: "danger" });
                        var resultJson = {};
                        resultJson.Rows = [];
                        resultJson.sEcho = 1;
                        resultJson.Total = 0;
                ...
                "fnServerParams": function (aoData) {  // 增加自定义查询条件
                    ...
                    aoData.push({ "name": "CarType", "value": $scope.SearchKey.CarType });
        ...
        $scope.export = function () {
            ...
            $http({
                method: 'get',
                url: '/Portal/ProductCenter/ProductCarTypeQueryExport?ProductName=' + $scope.SearchKey.ProductName + "&CarType=" + $scope.SearchKey.CarType,
                headers: { 'Content-Type': 'application/vnd.ms-excel' },
                responseType: 'arraybuffer'
            }).success(function (result) {
                if (result == "" || result == null) {
                    $.notify({ message: "未查询到产品/车型数据", status: "danger" });
                }
                else {
