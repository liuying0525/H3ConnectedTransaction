app.controller('ProductDealerRemoveRelationController', ['$scope', "$rootScope", "$translate", "$timeout", "$compile",
    ...
    function ($scope, $rootScope, $translate, $timeout, $compile, $http, $state, $stateParams, $filter, ControllerConfig, $interval, jqdatables) {
        ...
        $scope.removeRelation = function () {
            ...
            $http({
                method: 'post',
                url: '/Portal/ProductCenter/RemoveRelation',
                data: JSON.stringify({ Products: products, Dealers: dealers }),
            }).success(function (respon) {
                if (respon.Code != 1) {
                    $.notify({ message: "解除关联成功", status: "success" });
                }
                else {
                    $.notify({ message: "解除关联失败", status: "danger" });
                }
