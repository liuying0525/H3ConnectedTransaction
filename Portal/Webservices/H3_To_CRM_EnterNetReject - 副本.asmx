...
public class H3_To_CRM_EnterNetReject : System.Web.Services.WebService
{
    ...
    public string DealerAccede = "INSERT INTO DEALERACCEDE(\"ObjectID\",\"CrmDealerId\",\"DealerName\",\"DealerAccount\")VALUES([VALUES])";
    [WebMethod]
        ...
        if (!string.IsNullOrEmpty(objectid))
        {
            ...
            if (instance != null && instance.Tokens != null && instance.Tokens.Length > 1)
            {
                ...
                if (auditidea4 == "3")
                {
                    ...
                    if (auditidea4 == "3")
                    {
                        string sql = DealerAccede.Replace("[VALUES]","'"+Guid.NewGuid()+"','"+dealerid+"','"+dealername+"','"+relationaccount+"'");
                        string sql1 = "UPDATE I_ALLOWIN SET ZHTIME='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' WHERE OBJECTID='" + objectid + "'";
                        try
                        {
                            int i1 = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteNonQuery(sql);
                            int i2 = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteNonQuery(sql1);
                        }
                        catch(Exception e) { }
                    }
                }
