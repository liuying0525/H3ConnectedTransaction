...
public class H3_To_CRM_UpdateEnterNetApproval : System.Web.Services.WebService
{
    ...
    public string AllowFile = "ApprovalForm,CollectionTable,BusinessLicense,CreditDocuments,AuthorizationDocument,Photo,DiscountAgreement,CargoAgreement,HallPhoto,OfficePhoto,BusinessInformation,FinancialInformation,FinancialId,CooperationAgreement,RepresentativeId,PersonnelPhoto,Report,SiteContract,XFHDSPB,QuotaForm,SalesData,FinancialInstitutions,BankFlow,PersonalInformation,SecurityAgreement,CreditReport,QYCreditReport,QualificationInformation,QueryRecord,LegalPerson";
    [WebMethod]
    public void EnterNetUpdate(string instanceid, string objectid, string dealerid, string dealername, string auditoremail, string auditor, string infocode, string originatetime, string currenttime, string auditidea1, string auditidea2, string auditidea3, string auditidea4, string state)
    {
        ...
        if (state == "3")
        {
            try
            {
                ...
                {
                    if (field.Split(':')[0] == "UpdateContactInformation" || allowin.IndexOf(field.Split(':')[0], StringComparison.OrdinalIgnoreCase) > -1) continue;
                    allowin += AllowFile.IndexOf(field.Split(':')[0],StringComparison.OrdinalIgnoreCase) > -1 ? "" : (allowin == "" ? field.Split(':')[0] : "," + field.Split(':')[0]) + "='" + update.Rows[0][field.Split(':')[0]] + "'";
                ...
                DataTable dt = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteDataTable("SELECT * FROM I_ALLOWIN WHERE CRMDEALERID='" + dealerid + "'");
                sql = "UPDATE DEALERACCEDE SET \"DealerId\" = '" + dt.Rows[0]["FINANCIALCODE"] + "' WHERE \"CrmDealerId\" = '" + dealerid + "'";
                int t = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteNonQuery(sql);
                /* 2019-04-18 string citylevel = "";
                if(!string.IsNullOrWhiteSpace(dt.Rows[0]["CITY"].ToString()))
                {
                    string city = dt.Rows[0]["CITY"].ToString().Replace("市", "");
                    citylevel = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteScalar("SELECT Category FROM OT_EnumerableMetadata WHERE CODE LIKE '%" + city +"%'") + string.Empty;
                }
                if (string.IsNullOrWhiteSpace(citylevel)) citylevel = "六级";
                sql = "INSERT INTO DEALERDATA(\"ObjectID\",\"CrmDealerId\",\"Name\",\"Type\",\"Value\")VALUES('" + Guid.NewGuid() + "','" + dealerid + "','城市等级','Single','" + citylevel + "')";
                int k = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteNonQuery(sql);*/
                sql = "UPDATE DEALERDATA SET \"DealerType\" = '" + update.Rows[0]["TYPE"] + "',\"DealerKind\" = '" + update.Rows[0]["DISTRIBUTORTYPE"] + "',\"DealerId\" = '" + dt.Rows[0]["FINANCIALCODE"] + "' WHERE \"CrmDealerId\" = '" + dealerid + "'";
                int d = AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteNonQuery(sql);
            }
            catch (Exception ex)
            {
                AppUtility.Engine.LogWriter.Write("H3_修改更新数据：" + ex.Message);
            }
        }
